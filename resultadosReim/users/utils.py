...
def get_animales(request):
    ...
    start_base = 'SELECT e.id, e.nombre, count(e.id) AS cantidad FROM elemento e WHERE'
    final_base = ' e.id >10 AND e.id<26 '
    cursor.execute(start_base + final_base)
    animales = str(((len(cursor.fetchall())*40)+20))
    return animales 
